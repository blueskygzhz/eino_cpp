...
// 订单处理器：提取订单金额
class OrderProcessor : public Runnable<std::map<std::string, std::any>, std::map<std::string, std::any>> {
public:
    std::map<std::string, std::any> Invoke(
        ...
        // 从输入提取订单ID
        if (input.count("order_id")) {
            output["order_id"] = input.at("order_id");
            std::cout << "  订单ID: " << std::any_cast<std::string>(input.at("order_id")) << std::endl;
        }
        
        // 从输入提取订单金额
        if (input.count("amount")) {
            output["amount"] = input.at("amount");
            std::cout << "  订单金额: " << std::any_cast<double>(input.at("amount")) << std::endl;
        }
...
// 用户查询器：查询用户信息
class UserLookup : public Runnable<std::map<std::string, std::any>, std::map<std::string, std::any>> {
public:
    std::map<std::string, std::any> Invoke(
        std::shared_ptr<Context> ctx,
        const std::map<std::string, std::any>& input,
        ...
        if (input.count("user_id")) {
            std::string user_id = std::any_cast<std::string>(input.at("user_id"));
...
// VIP 服务处理器
class VIPService : public Runnable<std::map<std::string, std::any>, std::map<std::string, std::any>> {
public:
    std::map<std::string, std::any> Invoke(
        std::shared_ptr<Context> ctx,
        const std::map<std::string, std::any>& input,
        ...
        output["priority"] = std::string("HIGH");
...
// 普通服务处理器
class RegularService : public Runnable<std::map<std::string, std::any>, std::map<std::string, std::any>> {
public:
    std::map<std::string, std::any> Invoke(
        std::shared_ptr<Context> ctx,
        const std::map<std::string, std::any>& input,
        ...
        std::map<std::string, std::any> output = input;
        output["service_type"] = std::string("REGULAR");
        output["discount"] = 1.0;  // 无折扣
        output["priority"] = std::string("NORMAL");
...
// 新用户服务处理器
class NewUserService : public Runnable<std::map<std::string, std::any>, std::map<std::string, std::any>> {
public:
    std::map<std::string, std::any> Invoke(
        std::shared_ptr<Context> ctx,
        const std::map<std::string, std::any>& input,
        ...
        std::map<std::string, std::any> output = input;
        output["service_type"] = std::string("NEW_USER");
...
void RunExample(const std::string& scenario_name,
    ...
    try {
        // 1. 创建 Graph
        auto graph = std::make_shared<Graph<std::map<std::string, std::any>, std::map<std::string, std::any>>>();
        ...
        // 4. 添加 BranchNode
        auto branch_node = std::make_shared<BranchNode<std::map<std::string, std::any>>>(branch_config);
        ...
        if (result.count("service_type")) {
            std::cout << "  服务类型: " << std::any_cast<std::string>(result["service_type"]) << std::endl;
        }
        if (result.count("discount")) {
            std::cout << "  折扣: " << std::any_cast<double>(result["discount"]) * 100 << "%" << std::endl;
        }
        if (result.count("priority")) {
            std::cout << "  优先级: " << std::any_cast<std::string>(result["priority"]) << std::endl;
        }
        if (result.count("welcome_gift")) {
...
int main() {
    std::cout << "\n╔════════════════════════════════════════════════════════════╗" << std::endl;
    std::cout << "║     BranchNode 多节点引用完整示例                         ║" << std::endl;
    std::cout << "║                                                            ║" << std::endl;
